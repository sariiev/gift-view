...
async def test_resolve_creates_object(session):
    ...
    name = "Plush Pepe"
    gift_id = await resolver.resolve_id(session=session, name=name)
...
async def test_resolve_uses_cache(session):
    ...
    name = "Plush Pepe"
    gift1_id = await resolver.resolve_id(session=session, name=name)
    gift2_id = await resolver.resolve_id(session=session, name=name)
